...
class DiscourseAlgolia
  ...
  def self.process_posts!(ids)
    ...
    # TODO: handle topics when post_number == 1
  ...
  def self.posts_index
    @posts_index ||= begin
      index = algolia_index("discourse-posts")
      index.set_settings(POSTS_SETTINGS) unless index.exists?
      index
    end
  end
  ...
  def self.users_index
    @users_index ||= begin
      index = algolia_index("discourse-users")
      index.set_settings(USERS_SETTINGS) unless index.exists?
      index
    end
  end
  ...
  def self.tags_index
    @tags_index ||= begin
      index = algolia_index("discourse-tags")
      index.set_settings(TAGS_SETTINGS) unless index.exists?
      index
    end
  end
  ...
  def self.algolia_index(name)
    @algolia_indexes[name] ||= algolia_client.init_index(name)
  end
